...
* Getting gene aliases and function descriptions from NCBI AnnotationHub _Brassica napus_ object and Ensembl release 56 Descriptions/Uniprot accession numbers
...
pathways_KEGG_GS<- pathways_KEGG$geneset

# Write terms for GSEA
write.csv(pathways_KEGG$geneset_name, "C:\\Users\\naata\\Documents\\MASTER\\TFM\\tfm\\KEGG_gs_descriptions.csv", row.names =FALSE)

# get gene names and transcript lengths when they exist
ann <- getBM(attributes=c("external_gene_name", "entrezgene_id"), mart=bnapus_mart, useCache = FALSE)

# get genesets but for external_gene_name identifiers
KEGG <-merge(pathways_KEGG_GS,ann,by.x="gene", by.y="entrezgene_id")

# Subset only external genes, save in separate file
KEGG_term2gene <- KEGG[,c(2,3)]
colnames(KEGG_term2gene) <- c("id","gene")
...
ml_KEGG <- KEGG %>%
  pivot_longer(gene, names_to = "Var", names_prefix = "Var") %>% 
  pivot_wider(names_from = id, values_from = value, names_sep = "", values_fn = list)
...
# SELECT SUBSET - NAME - GO AND FILTER
db_gs_ncbi <- select(org.Brassicanapus.eg.db, egid, c("GENENAME", "GO"), "ENTREZID")
gs_ncbi_filtered <- db_gs_ncbi[which(db_gs_ncbi[,3] != "<NA>"),]

eGO_NCBI <-merge(gs_ncbi_filtered,ann,by.x="ENTREZID", by.y="entrezgene_id")

# Save to file

write.csv(eGO_NCBI, "C:\\Users\\naata\\Documents\\MASTER\\TFM\\tfm\\NCBI_gn_annotation_gs_pairs.csv", row.names =FALSE)
...
# 1st gene descriptive annotation retrieval : NCBI and Ensembl

# National Centre of Biotecnology (NCBI) centric annotations
```{r}
# Gets gene function description GENENAME and short name ALIAS.

gene_name <- select(org.Brassicanapus.eg.db, egid, c("GENENAME", "ALIAS"), "ENTREZID")

colnames(gene_name)<-c("entrezgene_id", "external_gene_name", "Description_NCBI", "ALIAS")

# Write to csv

write.csv(gene_name, "C:\\Users\\naata\\Documents\\MASTER\\TFM\\tfm\\ncbi_centric_annotations.tsv", row.names =FALSE)
...
# Ensembl description annotations
```{r}
# GET ANNOTATIONS
ann_descriptions <- getBM(attributes=c("external_gene_name", "description"), mart=bnapus_mart, useCache = FALSE)


description_all <- lapply(ann_descriptions$description, function(x) {
  list1 <- unlist(str_split(x, "\\["))
  gsub("\\s$","", list1[1]) # description
  })


acc_uniprot_all <- lapply(ann_descriptions$description, function(x) {
  list1 <- unlist(str_split(x, "\\["))
  list2 <- unlist(str_split(list1[2], "\\;"))
  acc_uniprot <- gsub("Acc:|\\]", "", list2[2])
  acc_uniprot # 
  })

ensembl_all <- data.frame(external_gene_name = ann_descriptions$external_gene_name,
                          Description_Ensembl = unlist(description_all),
                          UniprotAcc = unlist(acc_uniprot_all))


write.table(ensembl_all, "C:\\Users\\naata\\Documents\\MASTER\\TFM\\tfm\\ensembl_centric_annotations.tsv", row.names = FALSE, quote=FALSE, sep="\t")
