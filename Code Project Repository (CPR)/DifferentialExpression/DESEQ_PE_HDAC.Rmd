...
```{r}
genes <- read.csv("/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/genes.csv", header=FALSE)

countTable <- read.csv("/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/count_matrix.csv")
colnames(countTable)<-c("VM1","VM2","VM3","T1","T2","T3")
...
rlog_normalized_counts <- rlog(counts(dds))

write.table(rlog_normalized_counts,"/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/Condition_T_vs_VM_normcounts_badj_DevStage_RlogTr.csv",quote=FALSE, sep=",")
...
#write.csv(Res_A_X_total, paste(directory, "/DE_results/Condition_T_vs_C_results_total.csv", sep=""))
#write.csv(Res_A_X_total_0.05, paste(directory, "/DE_results/Condition_T_vs_C_result_padj_0.05.csv", sep="")) 
...
write.table(final_df,"/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/Final_Condition_T_vs_VM_total.csv", quote=FALSE, sep=",", row.names = FALSE)

final_df_filtered <- subset(subset(final_df, padj<0.05),abs(log2FoldChange)>1)

write.table(final_df_filtered,"/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/Final_Condition_T_vs_VM_LFC1_padj05.csv", quote=FALSE, sep=",", row.names = FALSE)
```
...
# Repeat analysis with batch correction and covariate addition analysis

Samples are affected by sources of non-relevant technical variation (differences due to personel, time, sequencing platform, ...) . Although the same sequencing and library construction protocol is used for both projects, we are merging sequencing results from two different experiments with samples that have been treated and isolated by different technitians in different timepoints. Therefore, it is important to model a distribution of gene counts expected for each condition and correct for the variance accounted for by each batch/ sequencing project. This is done in the `batch_correction.R` R script that uses a variation of the Surrogate Variable Analysis and a fitting of a Generalized Binomial Model (GLM) in a RNA-seq workflow, and also avoid corrections that alter our biological covariates - relevant variance (in our case, Treatment and Developmental Stage). It is important to compare the analysis to our latter DEA, to see if any artifacts are possibly introduced during batch correction.

```{r}
# get adjusted counts ( obtained through batch correction script ) and subset only Treated proembryos and Vacuolated Microspore counts

data <- read.csv("/home/famgarcia/Escritorio/genes_all_experiments_batch_correction_woPErep3_bothcovariates.csv", sep=",")

# subset to samples of interest in boolean index

col_bindex <- grepl("Trep", colnames(data)) | grepl("VM", colnames(data))
data <- data[,col_bindex]
...
dds$condition <- factor(dds$condition, levels = c("T", "VM"))
dds$condition <- relevel(dds$condition, ref = "VM")
...
# normalize and calculate dispersion, fitType local as the dispersion trend is not well captured by de function y = a/x + b
dds <- DESeq(dds_filter, fitType="local") 

# Plot size factors estimation - should be similar per condition
...
# Repeat analysis with batch correction and only Development Stage covariate

Far too many artifacts are created correcting by . We expect differential expression to have more prominent upregulation in proembryos vs microvacuolated spores, not 
...
# plot a histogram of the log scaled counts
png("/home/famgarcia/Escritorio/Histlog10_VM_vs_T_beforefilter", width=12, height=12, units="cm", res=300)
plot_without_filter <- hist(logsumcounts,breaks=100, main="Histogram of the log scaled counts")
dev.off()
...
png("/home/famgarcia/Escritorio/Histlog10_VM_vs_T_afterfilter", width=12, height=12, units="cm", res=300)
plot_with_filter <- hist(logsumcounts,breaks=100,main="Histogram of the log scaled counts \n keep only the genes with summed counts greater than 10" )
dev.off()
...
png("/home/famgarcia/Escritorio/dispersionPlot_VM_vs_T.png", width=12, height=12, units="cm", res=300)
plotDispEsts(dds, main="Dispersion plot")
dev.off()
```
```{r}
...
write.table(normalized_counts_Stress,"/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/Condition_PEall_vs_VM_normcounts_badj_DevStage_MoR.csv",quote=FALSE, sep=",")

#rlog_normalized_counts_Stress <- rlog(counts(dds_Stress))

#write.table(rlog_normalized_counts_Stress,"/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/Condition_PEall_vs_VM_normcounts_badj_DevStage_RlogTr.csv",quote=FALSE, sep=",")
...
Res_A_X_total <- Res_A_X_total[order(Res_A_X_total_Stress$padj),]
...
# Reannotate transcripts
go_ann <- getBM(filter="ensembl_gene_id",value=rownames(Res_A_X_total),attributes=c("ensembl_gene_id", "external_gene_name","start_position", "end_position", "strand", "description"),bnapus_mart, useCache = FALSE)
...
write.table(normalized_counts,"/home/famgarcia/VM_vs_HDAC_DEA/Condition_T_vs_VM_normcounts_badj_DevStage_MoR.csv",quote=FALSE, sep=",")
rlog_normalized_counts = rlog(dds, blind=FALSE)
write.table(rlog_normalized_counts,"/home/famgarcia/VM_vs_HDAC_DEA/Condition_T_vs_VM_normcounts_badj_DevStage_rlog.csv",quote=FALSE, sep=",")

# normalized, variance-stabilized transformed counts for visualization
vst_normalized_counts <- vst(dds, blind=FALSE)
vst_normalized_counts <- assay(vst_normalized_counts)
write.table(vst_normalized_counts,"/home/famgarcia/VM_vs_HDAC_DEA/Condition_T_vs_VM_normcounts_badj_DevStage_vst.csv",quote=FALSE, sep=",")

write.csv(Res_A_X_total, "/home/famgarcia/VM_vs_HDAC_DEA/Condition_T_vs_VM_results_total_bce_DevStage.csv", )
write.csv(Res_A_X_total_0.05, "/home/famgarcia/VM_vs_HDAC_DEA/Condition_T_vs_VM_result_padj_0.05_DevStage.csv") 
...
# Save file with sorted ranking score - Wald statistic for ranking
rank_res <- Res_A_X_total_plot[,c(9,5)]
write.table(rank_res[order(rank_res$stat,decreasing=TRUE),],"/home/famgarcia/VM_vs_HDAC_DEA/Rank_T_vs_VM_bce_DevStage.rnk", row.names = FALSE, sep=",", quote=FALSE, col.names=FALSE)
```
...
final_df <- merge(Res_A_X_total_plot,lfc_df, by = "ensembl_gene_id")
final_df_filtered <- merge(Res_A_X_total_plot,lfc_df, by = "ensembl_gene_id")

write.table(final_df,"/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/Final_Condition_T_vs_VM_total_badj.csv", quote=FALSE, sep=",", row.names = FALSE)

final_df_filtered <- subset(subset(final_df, padj<0.05),abs(log2FoldChange)>1)

write.table(final_df_filtered,"/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/Final_Condition_T_vs_VM_LFC1_padj05_badj.csv", quote=FALSE, sep=",", row.names = FALSE)
```
## RNA-seq data quality assessment
...
svg("/home/famgarcia/VM_vs_HDAC_DEA/Treecorr_ME_HDAC_rlog.svg")
TreeC = as.dendrogram(hc, method="average")
plot(TreeC,
     main = "Sample Clustering-checking outliers ",
     ylab = "Height")
...
svg("/home/famgarcia/VM_vs_HDAC_DEA/HeatmapRLDcorr_ME_HDAC.svg")
heatmap.2(pearson_cor, Rowv=as.dendrogram(hc), symm=T,trace="none",
col=hmcol, main="Pearson correlation of each
sample")
dev.off()
```
```{r}
write.csv(pearson_cor, "/home/famgarcia/VM_vs_HDAC_DEA/pearsonRLD_ME_HDAC_bc_DevStage.csv")
```
...
svg("/home/famgarcia/VM_vs_HDAC_DEA/VST_PCA_ME_HDAC.svg")
plotPCA(vsd, intgroup="condition") + geom_text(aes(label=name),vjust=-1) +  ggtitle("PCA Plot using variance stabilizing transformation data") + scale_x_continuous(limits = c(-100, 100))  +  scale_y_continuous(limits = c(-50, 50)) # change limits to fix figure dimensions
dev.off()

svg("/home/famgarcia/VM_vs_HDAC_DEA/RLD_PCA_ME_HDAC.svg")
plotPCA(rld, intgroup="condition") + geom_text(aes(label=name),vjust=1.5) + ggtitle("PCA Plot using Regularized log  Transformation data") + scale_x_continuous(limits = c(-200, 200))  +  scale_y_continuous(limits = c(-100, 100)) # change limits to fix figure dimensions
dev.off()
...
```{r, warning = FALSE, message = FALSE, echo = FALSE, fig.width=5, fig.height=5}
png("/home/famgarcia/VM_vs_HDAC_DEA/PMA_Standard_PE_HDAC.png", width=12, height=12, units="cm", res=300)
plotMA(dds , alpha =0.05, main=("Standard Bland-Altman- MA Plot"))
dev.off()

resLFC <- lfcShrink(dds, coef="condition_T_vs_VM", type="apeglm")

png("/home/famgarcia/VM_vs_HDAC_DEA/PMA_ShrunkenME_HDAC.png", width=12, height=12, units="cm", res=300)
#You can see that genes with low counts are now shrinked.                  
plotMA(resLFC, alpha =0.05, ylim=c(-4,4) ,main= ("MA plot shrunken log2 fold changes"))
dev.off()
...
maplot = ggmaplot(resLFC, size = 1,
                  ...
                  font.legend = c("bold",12), font.main = "bold",
                  xlab = "Log2 Mean Expression",  ylab="Log2 FC")
png("/home/famgarcia/VM_vs_HDAC_DEA/MAPLOT_ME_HDAC_better.png", width=12, height=12, units="cm", res=300)
show(maplot)
dev.off()
```
...
data_up_g = data[(data$color == "Increased" ) ,]
data_up_g = data_up_g[data_up_g$pval >1.3,]
data_up_g = data_up_g[order(data_up_g$pval, decreasing = TRUE),]
data_up_g = data_up_g[1:10,]
data_up_g = data_up_g[order(data_up_g$lfc, decreasing = TRUE),]
data_up_g = data_up_g[1:5,]

data_down_g = data[(data$color == "Decreased" ) ,]
data_down_g = data_down_g[data_down_g$pval > 1.3,]
data_down_g = data_down_g[order(data_down_g$pval, decreasing = TRUE),]
data_down_g = data_down_g[1:10,]
data_down_g = data_down_g[order(data_down_g$lfc, decreasing = F),]
data_down_g = data_down_g[1:5,]
...
vol <- vol +   
  ggtitle(label = "Volcano Plot", subtitle = "Colored by fold-change direction") +
  geom_point(size = 0.4, alpha = 0.8, na.rm = T) +
  scale_color_manual(name = "Directionality",
                     values = c(Increased = "#CD4F39", Decreased = "#008B00", nonsignificant = "darkgray")) +
  scale_fill_discrete(labels=c('Increased : 17254 genes', 'Decreased : 15914 genes', 'nonsignificant : 35946 genes')) +
  theme_bw(base_size = 10) + # change overall theme
  theme(legend.position = "right") + # change the legend
  xlab(expression(log[2]("FoldChange"))) + # Change X-Axis label
  ylab(expression(-log[10]("adjusted p-value"))) + # Change Y-Axis label
  geom_hline(yintercept = -1.3, colour = "darkgrey", linetype = 2) + # Add p-adj value cutoff line
  geom_hline(yintercept = 1.3, colour = "darkgrey", linetype = 2) +
  geom_vline(xintercept = 1, colour = "darkgrey", linetype = 2) + # Add LFC value cutoff line
  geom_vline(xintercept = -1, colour = "darkgrey", linetype = 2) +
  scale_y_continuous(trans = "log1p", oob = scales::squish_infinite, limits = c(0,400)) # Scale yaxis due to large p-values

png("/home/famgarcia/VM_vs_HDAC_DEA/volcanoplot_VM_vs_T.png", width=15, height=9, units="cm", res=300)
vol + geom_label_repel(
    data = data_up_g,
...
```{r}
# retrieving NCBI Entrez gene id for our genes called differential
signi_genes_annotated<- biomartr::biomart(genes = Res_A_X_total_0.05[,1],                  # genes were retrieved using biomartr::getGenome()
                                mart = "plants_mart",                     # marts were selected with biomartr::getMarts()
                                dataset = "bnapus_eg_gene",               # datasets were selected with biomartr::getDatasets()
                                attributes = attributes_to_retrieve,            # attributes were selected with biomartr::getAttributes()
                                filters =   "ensembl_gene_id" )# query key
...
ora_analysis_bp_df <- ora_analysis_bp@result  %>% tibble::rownames_to_column("GO") 
readr::write_csv(ora_analysis_bp_df, "/home/famgarcia/VM_vs_HDAC_DEA/Condition_T_vs_VM_Bnapus_bp_analysis_res_bce_DevStage.csv")
...
# Repeat DEA with all proembryo samples from both experiments (VM vs regular PE)

More samples --> more statistical power. We will also analyze if batch effect correction data provides a different DEA and introduces artifacts in VM vs PE expression data
...
col_bindex <- grepl("Crep", colnames(data)) | grepl("VM", colnames(data)) | grepl("PE", colnames(data))
data <- data[,col_bindex]
data<-data[,c(1,2,3,7,8,4,5,6)]


condition <- data.frame(
  condition<-c("PE","PE","PE","PE","PE","VM","VM","VM")
  )
rownames(condition) <-c("Crep1","Crep2","Crep3", "PErep1", "PErep2", "VMrep1","VMrep2","VMrep3")
ncol(data) == nrow(condition)
...
dds$condition <- factor(dds$condition, levels = c("PE","VM"))
dds$condition <- relevel(dds$condition, ref = "VM")
...
# normalize and calculate dispersion, fitType local as the dispersion trend is not well captured by de function y = a/x + b
dds_Stress <- DESeq(dds_filter) 

# Plot size factors estimation - should be similar per condition
counts <- counts(dds_Stress, normalized = TRUE)
design <- as.data.frame(colData(dds_Stress))
...
```
```{r}
res_Stress <- results(dds_Stress, pAdjustMethod="BH") 

```

```{r}
boxplot(rlog(counts(dds)))
```
...
```{r}
plotDispEsts(dds_Stress, main="Dispersion plot")

```
```{r}
par(mar=c(8,5,2,2))
boxplot(log10(assays(dds_Stress)[["cooks"]]), range=0, las=2)
```
...
mcols(res_Stress, use.names = T)


```
```{r}
ressign<-subset(res_Stress,abs(log2FoldChange)>1)
sprintf('genes with padj<0.05 and |LFC|>1 : %s', nrow(ressign[which((ressign$padj < 0.05) & (!is.na(ressign$padj))),]))
sprintf('genes with padj<0.05 and LFC>1 (up) : %s', nrow(ressign[which((ressign$log2FoldChange>1 & ressign$padj < 0.05) & (!is.na(ressign$padj))),]))
sprintf('genes with padj<0.05 and LFC<-1 (down) : %s', nrow(ressign[which((!ressign$log2FoldChange>1) & ressign$padj < 0.05 & (!is.na(ressign$padj))),]))
```
...
##Create normalized read counts
normalized_counts <- counts(dds_Stress, normalized=TRUE)
...
Res_A_X_total_S <- results(dds_Stress,
name="condition_PE_vs_VM",
pAdjustMethod="BH")

Res_A_X_total_S <- Res_A_X_total_S[order(Res_A_X_total_S$padj),]
...
# get gene names and transcript lengths when they exist
ann <- getBM(filter="ensembl_gene_id",value=rownames(Res_A_X_total_S),attributes=c("ensembl_gene_id","chromosome_name", "external_gene_name","start_position", "end_position", "strand", "transcript_biotype","transcript_length", "description"),mart=bnapus_mart, useCache = FALSE)
...
# Reannotate transcripts
go_ann <- getBM(filter="ensembl_gene_id",value=rownames(Res_A_X_total_S),attributes=c("ensembl_gene_id","chromosome_name", "external_gene_name","start_position", "end_position", "strand", "transcript_biotype","transcript_length", "description"),bnapus_mart, useCache = FALSE)


nrow(Res_A_X_total_S)
nrow(ann)


Res_A_X_total_S <- data.frame(Res_A_X_total_S)
Res_A_X_total_S  <- rownames_to_column(Res_A_X_total_S, var = "ensembl_gene_id")


#nrow(ann)
#nrow(Res_A_X_total)

Res_A_X_total_S <-merge(Res_A_X_total_S,go_ann,by='ensembl_gene_id',all.x = TRUE, all.y = TRUE)

#Export output files
Res_A_X_total_S$sig <- ifelse(Res_A_X_total_S$padj <= 0.05, "yes", "no")

Res_A_X_total_0.05_S <-subset(Res_A_X_total_S, padj <=0.05)
...
write.csv(Res_A_X_total_S, "/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/Condition_PEall_vs_C_results_total_bce_DevStage.csv", )
write.csv(Res_A_X_total_0.05_S, "/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/Condition_PEall_vs_C_result_padj_0.05_DevStage.csv") 
...
```{r}
# Save file with sorted ranking score - Wald statistic for ranking
rank_res <- Res_A_X_total_S[,c(9,5)]
write.table(rank_res[order(rank_res$stat,decreasing=TRUE),],"/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/Rank_PEall_vs_VM_bce_DevStage.rnk", row.names = FALSE, sep=",", quote=FALSE, col.names=FALSE)

rank_res <- Res_A_X_total[,c(9,5)]
write.table(rank_res[order(rank_res$stat,decreasing=TRUE),],"/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/Rank_T_vs_VM_bce_DevStage.rnk", row.names = FALSE, sep=",", quote=FALSE, col.names=FALSE)
```
...
rld <- rlog(dds_Stress, blind=FALSE)
rlogMat <- assay(rld)
rlogMat <- rlogMat[order(normalized_counts_mad, decreasing=T), ]
...
```
```{r}
write.csv(pearson_cor, "/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/pearsonRLD_PEall_vs_VM_bc_DevStage.csv")
```
...
# normalized, variance-stabilized transformed counts for visualization
vsd <- vst(dds_Stress, blind=FALSE)

svg("/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/VST_PCA_VM_vs_PEall.svg")
plotPCA(vsd, intgroup="condition") + geom_point(size = 0.1) + geom_text(aes(label=name),vjust=-1, hjust=-0.09, size=4) +  ggtitle("PCA Plot using variance stabilizing transformation data") + scale_x_continuous(limits = c(-100, 200))  +  scale_y_continuous(limits = c(-50, 50)) # change limits to fix figure dimensions
dev.off()

svg("/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/RLD_PCA_VM_vs_PEall.svg")
plotPCA(rld, intgroup="condition")  + geom_point(size = 0.1) + geom_text(aes(label=name),vjust=1.5, hjust=1, size=4) + ggtitle("PCA Plot using Regularized log  Transformation data") + scale_x_continuous(limits = c(-200, 200))  +  scale_y_continuous(limits = c(-100, 100)) # change limits to fix figure dimensions
dev.off()
...
```{r}
# This plot gives an idea of the dispersion variability among genes that have high gene counts versus genes that do not. This will affect the LFC, which is why it is important to calculate a prior distribution with LFC that are expected not to change(for genes with high counts), and use it to shrink or predict new estimates of LFC values with reduced noise for genes with less counts.
plotSparsity(dds_Stress)
```

```{r, warning = FALSE, message = FALSE, echo = FALSE, fig.width=5, fig.height=5}
svg("/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/PMA_Standard_PEall_VM.svg")
plotMA(dds_Stress , alpha =0.05, main=("Standard Bland-Altman- MA Plot"))
dev.off()

resLFC <- lfcShrink(dds_Stress, coef="condition_PE_vs_VM", type="apeglm")

#svg("/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/PMA_Shrunken_VM_VS_PEall.svg")
#You can see that genes with low counts are now shrinked.                  
plotMA(resLFC, alpha =0.05, ylim=c(-4,4) ,main= ("MA plot shrunken log2 fold changes"))
#dev.off()
...
maplot = ggmaplot(resLFC, size = 1,
                  ...
                  font.legend = c("bold",12), font.main = "bold",
                  xlab = "Log2 Mean Expression",  ylab="Log2 FC")
svg("/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/MAPLOT_VM_vs_PEall_better.svg")
show(maplot)
dev.off()
```
# Save log2FoldChange shrink in separate excel
```{r}
genelist <- rownames(resLFC)
lfc_df = as.data.frame(resLFC[,2])
lfc_df$ensembl_gene_id = genelist
...
write.table(final_df,"/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/Final_Condition_PEall_vs_VM_total_badj.csv", quote=FALSE, sep=",", row.names = FALSE)

final_df_filtered <- subset(subset(final_df, padj<0.05),abs(log2FoldChange)>1)

write.table(final_df_filtered,"/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/Final_Condition_PEall_vs_VM_LFC1_padj05_badj.csv", quote=FALSE, sep=",", row.names = FALSE)
```
### Volcano plot {.tabset .tabset-fade .tabset-pills}
...
EnhancedVolcano(toptable = resLFC,      # We use the shrunken log2 fold change as noise associated with low count genes is removed 
                ...
                xlim = c(-5, 4),
                ylim = c(0,25),
                title="Condition PEall vs. VM", subtitle = NULL,
                lab = rownames(resLFC)) +scale_y_continuous(limits = c(0,25))
#dev.off()

# Gather Log-fold change and FDR-corrected pvalues from DESeq2 results

## - Change pvalues to -log10 (1.3 = 0.05)
data <- data.frame(gene = row.names(res_Stress),
                   pval = -log10(res_Stress$padj), 
                   lfc = res_Stress$log2FoldChange)
...
png("/home/famgarcia/TFM/data/Testillano col/MV vs PE_HDAC/VolcanoFiltering_VM_vs_PEall_bce_DevStage.png", res=400, height=20,width=20, units="cm")
# Make a basic ggplot2 object with x-y values
vol <- ggplot(data, aes(x = lfc, y = pval, color = color))
...
# Query the Ensembl API
all_arabidopsis_genes_annotated <- biomartr::biomart(genes = rownames(Res_A_X_total_S), mart= "plants_mart",   dataset = "bnapus_eg_gene",  attributes = attributes_to_retrieve , filters =  "ensembl_gene_id")  
# for compatibility with enrichGO universe
# genes in the universe need to be characters and not integers (Entrez gene id)
all_arabidopsis_genes_annotated$entrezgene_id = as.character(
  all_arabidopsis_genes_annotated$entrezgene_id)
...
```{r}
# retrieving NCBI Entrez gene id for our genes called differential
signi_genes_annotated<- biomartr::biomart(genes = Res_A_X_total_0.05_S[,1],                  # genes were retrieved using biomartr::getGenome()
                                mart = "plants_mart",                     # marts were selected with biomartr::getMarts()
                                dataset = "bnapus_eg_gene",               # datasets were selected with biomartr::getDatasets()
                                attributes = attributes_to_retrieve,      # attributes were selected with biomartr::getAttributes()
                                filters =   "ensembl_gene_id" )# query key
...
ora_analysis_bp_df <- ora_analysis_bp@result  %>% tibble::rownames_to_column("GO") 
readr::write_csv(ora_analysis_bp_df, "/home/famgarcia/VM_vs_HDAC_DEA/Condition_PEall_vs_VM_Bnapus_bp_analysis_res_bce_DevStage.csv")
...
ora_analysis_bp_df <- ora_analysis_bp@result  %>% tibble::rownames_to_column("GO") 
readr::write_csv(ora_analysis_bp_df, "/home/famgarcia/VM_vs_HDAC_DEA/Condition_PEall_vs_VM_Bnapus_cc_analysis_res_.csv")
...
ora_analysis_bp_df <- ora_analysis_bp@result  %>% tibble::rownames_to_column("GO")
readr::write_csv(ora_analysis_bp_df, "/home/famgarcia/VM_vs_HDAC_DEA/Condition_VM_vs_PE_Bnapus_mf_analysis_res_.csv")
