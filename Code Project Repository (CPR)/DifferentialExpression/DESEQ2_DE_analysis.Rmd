...
directory <- "/home/famgarcia/TFM/data/Testillano col/Report Novogene RNAseq Deacetilation Inhibition treatment vs Control/ANALYSIS NATALIA/counts"
...
sampleNames <- c("Crep1", "Crep2", "Crep3", "Trep1", "Trep2", "Trep3")
...
ddsHTSeq$condition <- factor(ddsHTSeq$condition, levels = c("C", "T"))
ddsHTSeq$condition

ddsHTSeq$condition <- relevel(ddsHTSeq$condition, ref = "C")
ddsHTSeq$condition
...
res <- results(dds, contrast = c('condition', "C","T"), pAdjustMethod="BH") 
...
write.csv(normalized_counts,paste(directory, "/DE_results/Condition_T_vs_C_normcounts.csv", sep=""))
...
#svg("PCA_VST_HDAC.svg")
plotPCA(vsd, intgroup="condition") + geom_text(aes(label=name),vjust=-1) +  ggtitle("PCA Plot using variance stabilizing transformation data") + scale_x_continuous(limits = c(-50, 50))  +  scale_y_continuous(limits = c(-20, 20))  # change limits to fix figure dimensions
#dev.off()

#svg("PCA_LTD_HDAC.svg")
plotPCA(rld, intgroup="condition") + geom_text(aes(label=name),vjust=1.5) + ggtitle("PCA Plot using Regularized log  Transformation data")+ scale_x_continuous(limits = c(-50, 50))  +  scale_y_continuous(limits = c(-20, 20))  # change limits to fix figure dimensions
#dev.off()
...
#svg("PMA_Standard_HDAC.svg")
plotMA(dds_new , alpha =0.05, main=("Standard Bland-Altman- MA Plot"))
#dev.off()

resLFC <- lfcShrink(dds_new, coef="condition_T_vs_C", type="apeglm")

#svg("PMA_Shrunken_HDAC.svg")
...
vol +   
  ...
  ylab(expression(-log[10]("Log10(adjusted p-value)"))) + # Change Y-Axis label
  #geom_hline(yintercept = 1.3, colour = "darkgrey") + # Add p-adj value cutoff line
  #geom_hline(xintercept = 1, colour = "darkgrey") + # Add p-adj value cutoff line
...
signi_genes_annotated<- biomartr::biomart(genes = Res_A_X_total_0.05$ensembl_gene_id,                  # genes were retrieved using biomartr::getGenome()
